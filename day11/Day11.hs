...
data Test = IsDivisibleBy Integer Int Int
          deriving (Show, Ord, Eq)
...
inspect :: Int -> Integer -> M.Map Int Monkey -> M.Map Int Monkey
inspect !src !r !ms = ms & at src % mapped %~ inspect'
  where
    inspect' m@(Monkey _ (i:is) op _) = m{items=(changeWorryLevel i op `div` r):is}
...
throw !src !ms =
  ...
  where
    throw' !i !dst =
      ms & at src % mapped % #items %~ drop 1
         & at dst % mapped % #items %~ (++[i])

step1 :: Int -> Integer -> M.Map Int Monkey -> M.Map Int Monkey
step1 !src !r = throw src . inspect src r

turn1 :: Integer -> Int -> M.Map Int Monkey -> M.Map Int Monkey
turn1 r src ms = case ms ^.. at src % traversed % #items of
  [is] -> iterate (step1 src r) ms !! length is
  _ -> ms

round1 :: Integer -> M.Map Int Monkey -> M.Map Int Monkey
round1 r ms =
  foldl' (&) ms (map (turn1 r) [0..M.size ms])

countInspected :: Int -> M.Map Int Monkey -> Integer
countInspected src ms = head (ms ^.. at src % traversed % #items % to (fromIntegral . length))

observe :: Integer -> Int -> M.Map Int Monkey -> IO (M.Map Int Integer)
...
part1 = do
  ...
  observed <- observe 3 20 monkeys
  let [m1, m2] = observed ^.. folded & sort & reverse & take 2
  print $ m1 * m2
  return ()

observe2 :: Integer -> Int -> M.Map Int Monkey -> M.Map Int Integer
observe2 relief numOfRound ms = round numOfRound ms (M.map (const 0) ms)
  where
    numOfTurn = M.size ms
    round 0 _ totalInspected = totalInspected
    round n ms' totalInspected =
      let (ms'', totalInspected') = turn 0 ms' totalInspected
      in
        round (n-1) ms'' totalInspected'
    turn !n !ms' !totalInspected
      | n == numOfTurn = (ms', totalInspected)
      | otherwise = turn (n+1) (turn1 relief n ms') (totalInspected & at n % mapped %!~ (+countInspected n ms'))

part2 :: IO ()
part2 = do
  monkeys <- load "day11/input_test.txt"
  let observed = observe2 1 1000 monkeys
  print $ observed ^.. traversed
